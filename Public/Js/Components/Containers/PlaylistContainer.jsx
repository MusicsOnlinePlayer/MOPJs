...
import { ChangePlayingId as ChangePlayingIdRedux, UpdateCurrentPlaylist as UpdateCurrentPlaylistRedux } from '../../Actions/Action';
...
class PlaylistContainerConnected extends React.Component {
	...
	onSortEnd = ({ oldIndex, newIndex }) => {
		...
		const newMusicsPlaylist = arrayMove(Musics, oldIndex, newIndex);
		...
		return (
			<PlaylistSortableContainer onSortEnd={this.onSortEnd} useDragHandle>
				{Musics.map((value, index) => (
					<PlaylistSortableElement key={`item-${value._id}`} index={index} value={{...value, index}} />
				))}
